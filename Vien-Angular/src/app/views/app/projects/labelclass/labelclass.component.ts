...
@Component({
  selector: 'app-labelclass',
  templateUrl: './labelclass.component.html'
})
export class LabelClassComponent implements OnInit, OnDestroy {
  ...
  GetLabelClasses() {
    ...
    this.authService.GetLabelClasses(loginId).subscribe((result: any) => {
        debugger;
      var finalresult = JSON.parse(result);
      if (finalresult.status == "200") {
        this.labelclasses = finalresult.result;
        this.rows = this.labelclasses.map(({ projectName,name, id }) => ({ projectName,name, id }));
        this.temp = [...this.rows];
      }
      else {
  ...
  confirm(): void {
    this.deleteLabelclass(parseInt(this.LabelclassId))
    this.modalRef.hide();
  }
  ...
  showAddNewModal() {
    this.router.navigate(['app/projects/addlabelclass']);
  }

  showEditModal(id) {
    var value = "edit";
    this.router.navigateByUrl('app/projects/editlabelclass/' + id + '/' + value);
  }
  showViewModal(id) {
    var value = "view";
    this.router.navigateByUrl('app/projects/editlabelclass/' + id + '/' + value);
  ...
  deleteLabelclass(id) {
    this.spinner.show();
    this.authService.DeleteLabelClass(id).subscribe((result: any) => {
      var data = JSON.parse(result);
      if (data.status == "200") {
        this.spinner.hide();
        this.notifications.success('Success', "Labelclass deleted successfully", NotificationType.Success, { theClass: 'outline primary', timeOut: 1000, showProgressBar: false });
        setTimeout(() => {
