...
export class ImportDataComponent implements OnInit, OnDestroy {
  ...
  GetDatasets() {
    ...
    this.authService.GetDatasets(loginId).subscribe((result: any) => {
      ...
      if (finalresult.status == "200") {
        ...
        this.rows = this.labelclasses.map(({ projectName,dataSourceName,dataTypeName,datasetName, id }) => ({ projectName,dataSourceName,dataTypeName,datasetName, id }));
        this.temp = [...this.rows];
      }
      else {
  ...
  confirm(): void {
    this.deleteDataset(parseInt(this.DatasetId))
    this.modalRef.hide();
  }
  ...
  showAddNewModal() {
    this.router.navigate(['app/projects/addimportdata']);
  }

  showEditModal(id) {
    var value = "edit";
    this.router.navigateByUrl('app/projects/editimportdata/' + id + '/' + value);
  }
  showViewModal(id) {
    var value = "view";
    this.router.navigateByUrl('app/projects/editimportdata/' + id + '/' + value);
  ...
  deleteDataset(id) {
    this.spinner.show();
    this.authService.DeleteDataset(id).subscribe((result: any) => {
      var data = JSON.parse(result);
      if (data.status == "200") {
        this.spinner.hide();
        this.notifications.success('Success', "Dataset deleted successfully", NotificationType.Success, { theClass: 'outline primary', timeOut: 1000, showProgressBar: false });
        setTimeout(() => {
