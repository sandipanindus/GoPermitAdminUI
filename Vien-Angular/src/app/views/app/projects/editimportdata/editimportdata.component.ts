import { Component, EventEmitter, OnInit, ViewChild, OnDestroy, Injectable, Input, Output } from '@angular/core';
...
@Component({
    selector: 'app-editimportdata',
    templateUrl: './editimportdata.component.html',
    styleUrls: ['./editimportdata.component.css']
})
export class EditImportDataComponent implements OnInit, OnDestroy {
    ...
    constructor(private approute: ActivatedRoute, private hotkeysService: HotkeysService, private apiService: ApiService, private spinner: NgxSpinnerService, private modalService: BsModalService, private formBuilder: FormBuilder,
    ...
    Edit(id: any, value: any) {
        this.spinner.show();
        if (value == "view") {
            (document.getElementById("txtprojectId") as HTMLInputElement).disabled = true;
            (document.getElementById("txtdatasetname") as HTMLInputElement).disabled = true;
            this.disabled = true;
           // (document.getElementById("txtnormalfile") as HTMLInputElement).disabled = true;
            (document.getElementById("txtftpurl") as HTMLInputElement).disabled = true;
            (document.getElementById("txtftpportno") as HTMLInputElement).disabled = true;
            (document.getElementById("txtftpusername") as HTMLInputElement).disabled = true;
            (document.getElementById("txtftppassword") as HTMLInputElement).disabled = true;
           // (document.getElementById("txtfilezip") as HTMLInputElement).disabled = true;
            (document.getElementById("txtsubmit") as HTMLButtonElement).disabled = true;
            
        }
        else {
            (document.getElementById("txtprojectId") as HTMLInputElement).disabled = false;
            (document.getElementById("txtdatasetname") as HTMLInputElement).disabled = false;
            this.disabled = false;
          //  (document.getElementById("txtnormalfile") as HTMLInputElement).disabled = false;
            (document.getElementById("txtftpurl") as HTMLInputElement).disabled = false;
            (document.getElementById("txtftpportno") as HTMLInputElement).disabled = false;
            (document.getElementById("txtftpusername") as HTMLInputElement).disabled = false;
            (document.getElementById("txtftppassword") as HTMLInputElement).disabled = false;
           // (document.getElementById("txtfilezip") as HTMLInputElement).disabled = false;
        ...
        this.search = "";
        this.DatasetId = id;
        this.authService.GetDatasetResponse(id, 10, 1, "")
            .subscribe((data: any) => {
                debugger;
                var result = JSON.parse(data);
                if (result.status == "200") {
                    if (result.result.data.length > 0) {
                        this.projectId=result.result.projectId;
                        this.datasourcename=result.result.datasourceName;
                        this.datatypename=result.result.datatypeName;
                        this.datasetname=result.result.datasetname;
    ...
    SaveImportData() {
        ...
        if (this.datasourcename.trim().toLowerCase() == "Image Upload".toLowerCase()) {
            ...
            this.authService.SaveFtpData(data).subscribe((data: any) => {
                ...
                if (result.status == "200") {
                    this.spinner.hide();
                    this.notifications.success('Success', "Ftp added successfully", NotificationType.Success, { theClass: 'outline primary', timeOut: 3000, showProgressBar: false, clickToClose: true });
    ...
    onChangeItemsPerPage(item) {
        ...
        this.loaddatasetdata(Id, item, 1, "");
    }
    pageChanged(event: any): void {
        //this.loadData(this.itemsPerPage, event.page, this.search, this.orderBy);
        var Id = this.DatasetId

        this.loaddatasetdata(Id, this.itemsPerPage, event.page, "");
    }
    ...
    onSearchKeyUp(event) {
        ...
        this.loaddatasetdata(Id, this.itemsPerPage, 1, val);
    }
