 ...
 @Component({
  selector: 'app-operatordetailstable',
  templateUrl: './operatordetailstable.component.html',
 ...
 agent
   ...
   showAddNewModal() {
     this.router.navigate(['app/operatorconfig/addoperatordetails']);
   }
   ...
   showEditModal(id) {
     ...
     this.router.navigateByUrl('app/operatorconfig/editoperatordetail/' + id + '/' + value);
   }
   showViewModal(id) {
     var value = "view";
     this.router.navigateByUrl('app/operatorconfig/editoperatordetail/' + id + '/' + value);
   ...
   deleteSite(id) {
     ...
     this.authService.DeleteOperatorid(id).subscribe((result: any) => {
       var data = JSON.parse(result);
       if (data.status == "200") {
         element.style.display = 'none';
         this.onSuccess("Operator deleted successfully");
