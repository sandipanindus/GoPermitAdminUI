...
export class DatasourceComponent implements OnInit, OnDestroy {
  ...
  GetDataSources() {
    ...
    this.authService.GetDataSources(loginId).subscribe((result: any) => {
      ...
      if (finalresult.status == "200") {
        ...
        this.rows = this.datasources.map(({ name, id }) => ({ name, id }));
        this.temp = [...this.rows];
      }
      else {
  ...
  confirm(): void {
    this.deleteDatasource(parseInt(this.DatasourceId))
    this.modalRef.hide();
  }
  ...
  showAddNewModal() {
    this.router.navigate(['app/appconfig/adddatasource']);
  }

  showEditModal(id) {
    var value = "edit";
    this.router.navigateByUrl('app/appconfig/editdatasource/' + id + '/' + value);
  }
  showViewModal(id) {
    var value = "view";
    this.router.navigateByUrl('app/appconfig/editdatasource/' + id + '/' + value);
  ...
  deleteDatasource(id) {
    ...
    this.authService.DeleteDataSource(id).subscribe((result: any) => {
      ...
      if (data.status == "200") {
        this.spinner.hide();
        this.notifications.success('Success', "Datasource deleted successfully", NotificationType.Success, { theClass: 'outline primary', timeOut: 1000, showProgressBar: false });
        setTimeout(() => {
