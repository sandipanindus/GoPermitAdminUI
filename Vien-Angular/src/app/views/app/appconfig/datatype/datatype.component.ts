...
export class DatatypeComponent implements OnInit, OnDestroy {
  ...
  GetDataTypes() {
    ...
    this.authService.GetDataTypes(loginId).subscribe((result: any) => {
      ...
      if (finalresult.status == "200") {
        ...
        this.rows = this.datatypes.map(({ name, id }) => ({ name, id }));
        this.temp = [...this.rows];
      }
      else {
  ...
  confirm(): void {
    this.deleteDatatype(parseInt(this.DatatypeId))
    this.modalRef.hide();
  }
  ...
  showAddNewModal() {
    this.router.navigate(['app/appconfig/adddatatype']);
  }

  showEditModal(id) {
    var value = "edit";
    this.router.navigateByUrl('app/appconfig/editdatatype/' + id + '/' + value);
  }
  ...
  deleteDatatype(id) {
    this.spinner.show();
    this.authService.DeleteDataType(id).subscribe((result: any) => {
      var data = JSON.parse(result);
      if (data.status == "200") {
        this.spinner.hide();
        this.notifications.success('Success', "Datatype deleted successfully", NotificationType.Success, { theClass: 'outline primary', timeOut: 1000, showProgressBar: false });
        setTimeout(() => {
