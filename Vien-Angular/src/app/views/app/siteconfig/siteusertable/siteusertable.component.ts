...
@Component({
  selector: 'app-siteusertable',
  templateUrl: './siteusertable.component.html',
  styles: [
...
export class SiteusertableComponent implements OnInit, OnDestroy {
  sitename = '';
  email = '';
  mobileno = '';
  label = "menu.site";
...
agent
  ...
  GetSites() {
    ...
    this.authService.GetSiteUser(this.currentPage, this.itemsPerPage, loginId, RoleId, SiteId).subscribe((result: any) => {
      var finalresult = JSON.parse(result);
      if (finalresult.status == "200") {
        debugger;
        this.sites = finalresult.result;       
  ...
  showAddNewModal() {
    this.router.navigate(['app/siteconfig/addsiteuser']);
  }
  ...
  showEditModal(id) {
    ...
    this.router.navigateByUrl('app/siteconfig/editsiteuser/' + id + '/' + value);
  }
  showViewModal(id) {
    var value = "view";
    this.router.navigateByUrl('app/siteconfig/editsiteuser/' + id + '/' + value);
