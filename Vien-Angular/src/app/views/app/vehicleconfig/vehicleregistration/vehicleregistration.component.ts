...
export class VehicleRegistrationComponent implements OnInit, OnDestroy {
  ...
  GetVehicleRegistrations() {
    ...
    this.authService.GetVehicleDetails().subscribe((result: any) => {
      var finalresult = JSON.parse(result);
      if (finalresult.status == "200") {
        this.datasources = finalresult.result;
        this.rows = this.datasources.map(({ parkingBay,firstName,lastName,siteName, id }) => ({ parkingBay,firstName,lastName,siteName, id }));
        this.temp = [...this.rows];
      }
      else {
  ...
  confirm(): void {
    this.deleteVehicleRegistration(parseInt(this.VehicleRegistrationId))
    this.modalRef.hide();
  }
  ...
  showAddNewModal() {
    this.router.navigate(['app/vehicleconfig/addvehicleregistration']);
  }

  showEditModal(id) {
    var value = "edit";
    this.router.navigateByUrl('app/vehicleconfig/editvehicleregistration/' + id + '/' + value);
  }
  showViewModal(id) {
    var value = "view";
    this.router.navigateByUrl('app/vehicleconfig/editvehicleregistration/' + id + '/' + value);
  ...
  deleteVehicleRegistration(id) {
    this.spinner.show();
    this.authService.DeleteDataSource(id).subscribe((result: any) => {
      var data = JSON.parse(result);
      if (data.status == "200") {
        this.spinner.hide();
        this.notifications.success('Success', "Vehicle Registration deleted successfully", NotificationType.Success, { theClass: 'outline primary', timeOut: 1000, showProgressBar: false });
        setTimeout(() => {
